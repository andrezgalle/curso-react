...
const OlvidePassword = () => {
  ...
  return (
    ...
    <form className="my-10 bg-white shadow rounded-lg px-10 py-10" onSubmit={handleSubmit}>
      ...
      <input type="submit" className="mb-5 bg-sky-700 w-full py-3 text-white uppercase font-bold rounded hover:cursor-pointer hover:bg-sky-800 transition-colors" value="Enviar Instrucciones"/>
    </form>

    <nav className="lg:flex lg:justify-between">
      <Link to={'/registrar'} className="block text-center my-5 text-slate-500 uppercase text-sm ">¿No tienes un cuenta? Regístrate</Link>
    </nav>
  </>
  )
}
